...
if ($ARGV[1]=~/\w/) { $mass_error=$ARGV[1];} else { $mass_error=10; }
if ($ARGV[2]=~/\w/) { $modifications=$ARGV[2];} else { $modifications=""; }
		...
		while ($pepmod=<IN>)
		{
			...
			if ($pepmod=~/^([^\t]+)\t([^\t]*)$/)
			{
				...
				for(my $k=$charge_min;$k<=$charge_max;$k++)
				{
					...
					if ($mod_count>0)
					{
						for(my $l=length($pep__);($l = rindex $pep__,$mod_aa[0],$l)>=0;$l--)
						{
							$this_mod_count++;
						}
					}
					for(my $l=0;$l<=$this_mod_count;$l++)
					{
						if ($l>0) 
						{
							if ($mod_mod[0]=~/^[A-Z]/)
						...
						my $mz=calc_pep_mono_mz($pep__,$k,"",$modifications_)-1.007276/$k+$mod/$k;
						for(my $m=0;$m<=3;$m++)
						{
							my $mz_=$mz*(1-$mass_error/1e+6);
							my $mz__=$mz*(1+$mass_error/1e+6);
							my $modifications__=$modifications_+$mod;
							print OUT qq!$pep__\t$modifications__\t$k\t$mz\t500\t$m\tP\t0\t1000000\t$mz_\t$mz__\n!;
							$mz+=1.007276/$k;
						}
						$peptides_count_++;
					}
