...
if ($ARGV[0]=~/\w/) { $filename=$ARGV[0];} else { $filename="peptide_list.txt"; }
...
if ($ARGV[3]=~/\w/) { $modifications=$ARGV[3];} else { $modifications="O\@M"; }
if ($ARGV[4]=~/\w/) { $charge_min=$ARGV[4];} else { $charge_min=1; }
if ($ARGV[5]=~/\w/) { $charge_max=$ARGV[5];} else { $charge_max=4; }
if ($ARGV[6]=~/\w/) { $residues_to_remove=$ARGV[6];} else { $residues_to_remove=0; }
...
if (open (IN,"$filename"))
{
	if (open(OUT,">$filename_.processed.txt"))
	{
		...
		{
			chomp($pep);
			for($i=0;$i<=$residues_to_remove;$i++)
			{
				my $pep_ = substr $pep,$i;
				for($j=0;$j<=$residues_to_remove;$j++)
				{
					my $pep__ = substr $pep_,0,length($pep_)-$j;
					for($k=$charge_min;$k<=$charge_max;$k++)
					{
						...
						if ($mod_count>0)
						{
							for($l=length($pep__);($l = rindex $pep__,$mod_aa[0],$l)>=0;$l--)
							{
								$this_mod_count++;
							}
						}
						for($l=0;$l<=$this_mod_count;$l++)
						{
							if ($l>0) 
							{
								if ($mod_mod[0]=~/^[A-Z]/)
								{
									$modifications_="$mod_mod[0]$l"; 
								}
								else
								{
									my $mod_mod_=$mod_mod[0]*$l;
									$modifications_="$mod_mod_"; 
								}
							} else { $modifications_=""; }
							my $mz=calc_pep_mono_mz($pep__,$k,$fixed_modifications,$modifications_)-1.007276/$k;
							for($m=0;$m<=3;$m++)
							{
								my $mz_=$mz*(1-$mass_error/1e+6);
								my $mz__=$mz*(1+$mass_error/1e+6);
								print OUT qq!$pep__\t$modifications_\t$k\t$mz\t500\t$m\tP\t0\t1000000\t$mz_\t$mz__\n!;
								$mz+=1.007276/$k;
							}
							$peptides_count_++;
						}
