...
if ($status=~/^AddProject$/)
{
	...
	close(IN);
	open(IN,qq!$SETTINGS{'DATA'}/projects_deleted.dat!);
	while($line=<IN>)
	{
		if($line=~/\>(.*)$/) 
		{ 
			$names_del{$project_names{$1}}="1"; 
		}
	}
	close(IN);
	
	############# same project names allowed in different sub folders #####
	my $flag=0;
	GetProjects(-1,-1);
	$dir="$SETTINGS{'DATA'}/$PROJECT_PATH{$project_id}/$project_name";
	if(-d "$dir")
	{
		$flag=1; 
	}
	############# end #####
	if ($project_name=~/\w/ and ($names{$project_name}!~/\w/ or $names_del{$project_name}=~/\w/ or $flag==0))
	{
		$project_id=AddProject($project_id,$project_name);
		$message=qq!<font color="#FF0000"><b>Project added</b></font><p>\n!;
	}
	elsif($names{$project_name}=~/\w/ and $names_del{$project_name}!~/\w/ and $flag==1)
	{
...
if ($status=~/^ViewQueue$/)
{
	...
	GetDataAnalysisList($project_id,"$SETTINGS{'TASKS'}/running/0");
	...
	%DATA_ANALYSIS_LIST=();
	GetDataAnalysisList($project_id,"$SETTINGS{'TASKS'}/running/1");
	my $running=$DATA_ANALYSIS_LIST{$project_id};
	while($running=~s/^#([^#]+)#//)
	{
		my $job=$1;
	...
	GetDataAnalysisList($project_id,"$SETTINGS{'TASKS'}/done");
	...
	foreach my $value(reverse sort keys %unsorted) 
	{
		...
		open(IN,qq!$SETTINGS{'TASKS'}/done/$unsorted{$value}!);
		while($line=<IN>)
		{
			if($line=~/\<result dir="(.*)"\/\>/) 
			{
...
if($status=~/^ViewAnalysis/)
{
	...
	{
		if(open(IN,qq!$SETTINGS{'TASKS'}/running/0/$file!))
		{
			print qq!<pre>\n!; 
			while($line=<IN>)
			{ 
		...
		if(open(IN,qq!$SETTINGS{'TASKS'}/running/1/$file!))
		{
			print qq!<pre>\n!; 
			while($line=<IN>)
			{ 
...
if ($status=~/^ViewTools$/)
{
		print qq!<a href="http://openslice.fenyolab.org/cgi-bin/alignment_cgi.pl">Alignment</a><br>!;
}
...
if ($status=~/^SubmitJobSteps$/)
{
	...
	GetDataAnalysisList($project_id,"$SETTINGS{'DATA'}/$PROJECT_PATH{$project_id}/todo");
	my $todo=$DATA_ANALYSIS_LIST{$project_id};
	while($todo=~s/^#([^#]+)#//)
	{
		$filename=$1; 
	...
	GetDataAnalysisList($project_id,"$SETTINGS{'DATA'}/$PROJECT_PATH{$project_id}/running/0");
	my $running=$DATA_ANALYSIS_LIST{$project_id};
	while($running=~s/^#([^#]+)#//)
	{
		$filename=$1; 
	...
	%DATA_ANALYSIS_LIST=();
	GetDataAnalysisList($project_id,"$SETTINGS{'DATA'}/$PROJECT_PATH{$project_id}/running/1");
	my $running=$DATA_ANALYSIS_LIST{$project_id};
	while($running=~s/^#([^#]+)#//)
	{
		$filename=$1; 
...
if ($status=~/^ViewExperiment$/)
{
	...
	######### new change - report page begins #############
	my $reportfile=$query->param("reportfile");
	my $local=$SETTINGS{'LOCAL'};
	$local=~s/([\/]+)/\\/g;
	my $remote="$SETTINGS{'RESULT'}/$PROJECT_PATH{$project_id}";
	$remote=~s/([\/]+)/\\\\/g;
	my $remote_="$SETTINGS{'RESULT'}$PROJECT_PATH{$project_id}";
	
	if (opendir(dir,"$SETTINGS{'LOCAL'}"))
	{
		@files=readdir dir;
		closedir dir;
	}
	...
	{	
		if ($file=~/report.*\.html/i and $file ne $reportfile)
		{	
			system(qq!del $file!);
		}	
		if ($file eq $reportfile)
		{
			system(qq!move "$local\\$file" "$remote" > move.log!);
			print qq!<font color="#FF0000"><b>Report saved successfully</b></font><p>\n!;	
			my $report_dir=GenerateRandomStringDir(); 
			copy("$remote_/$file","$report_dir/index.html");
			if($report_dir=~/$SETTINGS{'REPORTS'}\/(.*)/)
			{
				open(REPORT,qq!>>$SETTINGS{'DATA'}/report.log!);
				print REPORT qq!$reportfile\t$1\n!;
				close(REPORT);
			}
		}
	}
	system(qq!del move.log!); 
	########## new change - report page ends ###############
	...
	if ($EXPERIMENT_DEFINITIONS{$project_id}=~/\w/)
	{
		...
		if ($done=~/\w/) 
		{ 
			print qq!<FORM ACTION="pipeline_cgi_reports.pl" METHOD="post" ENCTYPE="multipart/form-data">!; 
	...
	my $found_=0;
	%REPORT_LIST=();
	GetReportList($project_id,"$SETTINGS{'RESULT'}/$PROJECT_PATH{$project_id}");
	if ($REPORT_LIST{$project_id}=~/\w/) { $found_=1; }
	if($found_==1)
	{
		print qq!<p><b>Reports</b>!;
		print qq! (Reports saved in $PROJECT_PATH{$project_id} on Isilon)</p>!;	
		open(IN_REPORT,qq!$SETTINGS{'DATA'}/report.log!);
		while($line=<IN_REPORT>)
		{
			if($line=~/([^\t]+)\t([^\t]+)/) { $report_hash{"$1"}="$2"; }
		}
		close(IN_REPORT);
		while($REPORT_LIST{$project_id}=~s/^#([^#]+)#//)
		{
			print qq!<a href="/pipeline/$SETTINGS{'OWNER'}/$PROJECT_PATH{$project_id}/$1">$1</a>!;
			
			print qq!(Customer Link: <a href="/reports/$report_hash{"$1"}/index.html">$report_hash{"$1"}</a>)<br>!;
		}
		print qq!<hr width=75% align=left>!;
	} 
	########## new change - report page ends ###############
...
if($status=~/DeleteAnalysisFile/)
{
	...
	$todo_data="$SETTINGS{'DATA'}\/$PROJECT_PATH{$project_id}\/$dir\/$file_name";
	$todo_data=~s/\//\\/g;
	if($dir eq "todo")
	{
	...
	$running_dev="$SETTINGS{'TASKS'}/$dir/0/$file_name";
	$running_dev=~s/\//\\/g;
	$running_data="$SETTINGS{'DATA'}/$PROJECT_PATH{$project_id}/$dir/0/$file_name";
	$running_data=~s/\//\\/g;
	if($dir eq "running")
	{
	...
	$running_dev="$SETTINGS{'TASKS'}/$dir/1/$file_name";
	$running_dev=~s/\//\\/g;
	$running_data="$SETTINGS{'DATA'}/$PROJECT_PATH{$project_id}/$dir/1/$file_name";
	$running_data=~s/\//\\/g;
	if($dir eq "running")
	{
		system(qq!del "$running_dev" /Q!);
...
if ($status=~/^DataAnalysisInputFiles$/)
{
	...
	if($step_select=~/ID \(xtandem\.pl.*/)
	{
		...
		{
			print qq!$i. $data_files[$i-1]!; 
			print qq! <a href="http://$SETTINGS{'XTANDEM'}/thegpm-cgi/plist.pl?npep=0&path=$SETTINGS{'RESULT'}/$res_dir$dataanalysis_date/$data_files[$i-1]&proex=-1&ltype=2">X\! Tandem</a> !;
			my $data_files_=$data_files[$i-1];
			$data_files_=~s/\.xml//i;
			if (open(TEST,"$SETTINGS{'RESULT'}/$res_dir/$dataanalysis_date/plots/id/base_peak/$data_files_.mgf.basepeak.txt.labelled.basepeak.png"))
			{
				close(TEST);
				print qq!(<a href="pipeline_cgi.pl?dataanalysis_date=$dataanalysis_date&dataanalysis_name=$dataanalysis_name&step_select=$step_select&data_files=$data_files[$i-1]&status=BasePeak&project_id=$project_id">Basepeak Chromatogram</a>)!;   
			}
			print qq!<br>\n!;
		}
	}	
...
{
	my $project_id=$query->param("project_id");
	my $maxtime=$query->param("maxtime");
	my $mintime=$query->param("mintime");
	my $maxint=$query->param("maxint");
	my $minint=$query->param("minint");
	my $labelpep=$query->param("label1");
	my $labelprot=$query->param("label2");
	my $labeldesc=$query->param("label3"); 
	my $peakcount=$query->param("peakcount");
	GetProjects($project_id,-1);
	my $res_dir=""; 
	my $root_name=$SETTINGS{'RESULT'};
	$root_name=~s/.*\/([^\/]+)$/$1/; 
	...
	my $step_select=$query->param("step_select");
	my $data_files=$query->param("data_files");
	my $data_files_=$data_files;
	$data_files_=~s/\.xml//i; 
	print qq!<p><b>$dataanalysis_date: $dataanalysis_name.xml<br>$step_select<br>$data_files: Basepeak Chromatogram<br></b><p>!;
	################  zooming starts here ##############
	my $file_paths = "$SETTINGS{'RESULT'}\/$dir$dataanalysis_date\/plots\/id\/base_peak";
	if($peakcount!~/\w/)
	{
		$peakcount_=25;
	}
	...
	my $line_counter=0;
	open(IN,qq!$file_paths/$data_files_.mgf.basepeak.txt.labelpeak_.basepeak.txt!);
	open(OUT,qq!>$file_paths/$data_files_.mgf.basepeak.txt.labelpeak.basepeak.txt!);
	print OUT qq!maxpeptides\tmaxintensity\tmaxtime\n!;
	while($line=<IN>)
	{
		if($line_counter<=$peakcount_ and $labeldesc eq "on"  and $labelprot eq "" and $labelpep eq "" and $line!~/maxpeptides\tmaxintensity\tmaxtime/)
		{
			if($line=~/([A-Za-z]+)\s+([^\s]+)\s([^\t]+)\t([^\t]+)\t([^\t]+)/)
			{
				print OUT qq!$3\t$4\t$5!;
			}
			else
		...
		elsif($line_counter<=$peakcount_ and $labelpep eq "on"  and $labelprot eq "" and $labeldesc eq "" and $line!~/maxpeptides\tmaxintensity\tmaxtime/)
		{
			if($line=~/([A-Za-z]+)\s+([^\t]+)\t([^\t]+)\t([^\t]+)/)
			{
				print OUT qq!$1\t$3\t$4!;
		...
		elsif($line_counter<=$peakcount_ and $labelprot eq "on"  and $labelpep eq "" and $labeldesc eq "" and $line!~/maxpeptides\tmaxintensity\tmaxtime/)
		{
			if($line=~/([A-Za-z]+)\s+([^\s]+)\s([^\t]+)\t([^\t]+)\t([^\t]+)/)
			{
				print OUT qq!$2\t$4\t$5!;
		...
		elsif($line_counter<=$peakcount_ and $labelprot eq "on" and $labelpep eq "on" and $labeldesc eq "" and $line!~/maxpeptides\tmaxintensity\tmaxtime/)
		{
			if($line=~/([A-Za-z]+)\s+([^\s]+)\s([^\t]+)\t([^\t]+)\t([^\t]+)/)
			{
				print OUT qq!$1      $2\t$4\t$5!; 
			}
			else
			{
				print OUT qq!$line!;
		...
		elsif($line_counter<=$peakcount_ and $labelprot eq "on" and $labeldesc eq "on" and $labelpep eq "" and $line!~/maxpeptides\tmaxintensity\tmaxtime/)
		{
			if($line=~/([A-Za-z]+)\s+([^\s]+)\s([^\t]+)\t([^\t]+)\t([^\t]+)/)
			{
				print OUT qq!$2 $3\t$4\t$5!;
			}
			elsif($line=~/([A-Za-z]+)\s+([^\s]+)\t([^\t]+)\t([^\t]+)/)
			{
				print OUT qq!$2\t$3\t$4!;
		...
		elsif($line_counter<=$peakcount_ and $labelpep eq "on" and $labeldesc eq "on" and $labelprot eq "" and $line!~/maxpeptides\tmaxintensity\tmaxtime/)
		{
			if($line=~/([A-Za-z]+)\s+([^\s]+)\s([^\t]+)\t([^\t]+)\t([^\t]+)/)
			{
				print OUT qq!$1 $3\t$4\t$5!;
			}
			elsif($line=~/([A-Za-z]+)\s+([^\t]+)\t([^\t]+)\t([^\t]+)/)
			{
				print OUT qq!$1\t$3\t$4!;
		...
		elsif($line_counter<=$peakcount_ and $labelpep eq "on" and $labeldesc eq "on" and $labelprot eq "on" and $line!~/maxpeptides\tmaxintensity\tmaxtime/)
		{
			print OUT qq!$line!;
			$line_counter++;
		}
	...
	my $lower_int=0;
	my $upper_int=0;
	my $basepeak_image=$data_files_.$datetime;
	open(IN,qq!$file_paths/$data_files_.mgf.basepeak.txt.labelpeak_.basepeak.txt!);
	while($line=<IN>)
	{
	...
	foreach my $key (sort { $a <=> $b} keys %intensity_hash) 
	{
		$intensity_array[$intensity_count]=$key; 
		$intensity_count++;
	}
	$time_size=@time_array; 
	$intensity_size=@intensity_array; 
	if($mintime!~/\w/ and $maxtime!~/\w/)
	{
	...
	if($minint!~/\w/ and $maxint!~/\w/)
	{
		$lower_int=$intensity_array[0]-1;
		$upper_int=($intensity_array[$intensity_size-1]+1);
	}
	else
	{
		$lower_int=$minint;
	...
	print qq!
		<FORM ACTION="pipeline_cgi.pl" METHOD="post" ENCTYPE="multipart/form-data">
		<INPUT TYPE="hidden" NAME="status" VALUE="BasePeak">
		<INPUT TYPE="hidden" NAME="project_id" VALUE="$project_id">
		<INPUT TYPE="hidden" NAME="dataanalysis_date" VALUE="$dataanalysis_date">
		<INPUT TYPE="hidden" NAME="dataanalysis_name" VALUE="$dataanalysis_name">
		<INPUT TYPE="hidden" NAME="step_select" VALUE="$step_select">
		<INPUT TYPE="hidden" NAME="data_files" VALUE="$data_files">
		<b>Min Time:</b> <INPUT TYPE="text" NAME="mintime" VALUE="$lower_limit">
		&nbsp;&nbsp;&nbsp;<b>Max Time:</b> <INPUT TYPE="text" NAME="maxtime" VALUE="$upper_limit">&nbsp;&nbsp;&nbsp;<br>
		<b>Min Intensity:</b> <INPUT TYPE="text" NAME="minint" VALUE="$lower_int">
		&nbsp;&nbsp;&nbsp;<b>Max Intensity:</b> <INPUT TYPE="text" NAME="maxint" VALUE="$upper_int">&nbsp;&nbsp;&nbsp;<br>
		!;			
		if($labelpep eq "on" and $labelprot eq "" and $labeldesc eq "")
		{
			print qq!<b>Peptides:</b> <INPUT TYPE="checkbox" NAME="label1" VALUE="on" checked>&nbsp;&nbsp;&nbsp; !;
			print qq!<b>Proteins:</b> <INPUT TYPE="checkbox" NAME="label2" VALUE="on" >&nbsp;&nbsp;&nbsp; !;
			print qq!<b>Description:</b> <INPUT TYPE="checkbox" NAME="label3" VALUE="on" >&nbsp;&nbsp;&nbsp; <br>!;
		}
		elsif($labelprot eq "on" and $labelpep eq "" and $labeldesc eq "")
		{
			print qq!<b>Peptides:</b> <INPUT TYPE="checkbox" NAME="label1" VALUE="on" >&nbsp;&nbsp;&nbsp; !;
			print qq!<b>Proteins:</b> <INPUT TYPE="checkbox" NAME="label2" VALUE="on" checked>&nbsp;&nbsp;&nbsp; !;
			print qq!<b>Description:</b> <INPUT TYPE="checkbox" NAME="label3" VALUE="on" >&nbsp;&nbsp;&nbsp; <br>!;
		}
		elsif($labeldesc eq "on" and $labelprot eq "" and $labelpep eq "")
		{
			print qq!<b>Peptides:</b> <INPUT TYPE="checkbox" NAME="label1" VALUE="on" >&nbsp;&nbsp;&nbsp;!;
			print qq!<b>Proteins:</b> <INPUT TYPE="checkbox" NAME="label2" VALUE="on" >&nbsp;&nbsp;&nbsp; !;
			print qq!<b>Description:</b> <INPUT TYPE="checkbox" NAME="label3" VALUE="on" checked>&nbsp;&nbsp;&nbsp; <br>!;
		}
		elsif($labelpep eq "on" and $labelprot eq "on" and $labeldesc eq "")
		{
			print qq!<b>Peptides:</b> <INPUT TYPE="checkbox" NAME="label1" VALUE="on" checked>&nbsp;&nbsp;&nbsp;!;
			print qq!<b>Proteins:</b> <INPUT TYPE="checkbox" NAME="label2" VALUE="on" checked>&nbsp;&nbsp;&nbsp; !;
			print qq!<b>Description:</b> <INPUT TYPE="checkbox" NAME="label3" VALUE="on" >&nbsp;&nbsp;&nbsp; <br>!;
		}
		elsif($labelpep eq "on" and $labeldesc eq "on" and $labelprot eq "")
		{
			print qq!<b>Peptides:</b> <INPUT TYPE="checkbox" NAME="label1" VALUE="on" checked>&nbsp;&nbsp;&nbsp;!;
			print qq!<b>Proteins:</b> <INPUT TYPE="checkbox" NAME="label2" VALUE="on" >&nbsp;&nbsp;&nbsp; !;
			print qq!<b>Description:</b> <INPUT TYPE="checkbox" NAME="label3" VALUE="on" checked>&nbsp;&nbsp;&nbsp; <br>!;
		}
		elsif($labeldesc eq "on" and $labelprot eq "on" and $labelpep eq "")
		{
			print qq!<b>Peptides:</b> <INPUT TYPE="checkbox" NAME="label1" VALUE="on" >&nbsp;&nbsp;&nbsp;!;
			print qq!<b>Proteins:</b> <INPUT TYPE="checkbox" NAME="label2" VALUE="on" checked>&nbsp;&nbsp;&nbsp; !;
			print qq!<b>Description:</b> <INPUT TYPE="checkbox" NAME="label3" VALUE="on" checked>&nbsp;&nbsp;&nbsp; <br>!;
		}		
		elsif($labeldesc eq "on" and $labelprot eq "on" and $labelpep eq "on")
		{
			print qq!<b>Peptides:</b> <INPUT TYPE="checkbox" NAME="label1" VALUE="on" checked>&nbsp;&nbsp;&nbsp;!;
			print qq!<b>Proteins:</b> <INPUT TYPE="checkbox" NAME="label2" VALUE="on" checked>&nbsp;&nbsp;&nbsp; !;
			print qq!<b>Description:</b> <INPUT TYPE="checkbox" NAME="label3" VALUE="on" checked>&nbsp;&nbsp;&nbsp; <br>!;
	...
	print qq!
		<b>Peak label count:</b> <INPUT TYPE="text" NAME="peakcount" VALUE="$peakcount_"><br>
		<INPUT TYPE="submit" NAME="Zoom" VALUE="Zoom">
		</FORM>
	!;
	if(open(OUT2,qq!>$data_files_.Rinfile.txt!))
	{	
		print OUT2 qq!windows(width=8, height=8)
					...
					par(font=1)
					Datafile <- read.table("$file_paths/$data_files_.mgf.basepeak.txt.labelled.basepeak.txt",header=TRUE, sep="\t")
		attach(Datafile)
		plot((intensity) ~ time, data=Datafile, type="l",axes=TRUE, xlab="Time", ylab="Intensity", xlim=c($lower_limit,$upper_limit), ylim=c($lower_int,$upper_int))
		Datafile2 <- read.table("$file_paths/$data_files_.mgf.basepeak.txt.labelpeak.basepeak.txt",header=TRUE, sep="\t")
		attach(Datafile2)
		text(maxtime,(maxintensity),maxpeptides, adj = c(0,0.5),offset=3,col="firebrick", srt=90, cex=0.6)
		!;
		#print OUT2 qq!savePlot(filename="$file_paths/$data_files_.mgf.basepeak.txt.labelled.basepeak.png",type="png")!;
		print OUT2 qq!savePlot(filename="$file_paths/$basepeak_image.mgf.basepeak.txt.labelled.basepeak.png",type="png")!;
		close(OUT2);		
		system(qq!"C:\\R\\bin\\x64\\Rterm.exe" --no-restore --no-save < "$data_files_.Rinfile.txt" > "$data_files_.Routfile.txt" 2>&1!);
	}
	if(open(TEST,"$file_paths/$basepeak_image.mgf.basepeak.txt.labelled.basepeak.png"))
	{
		close(TEST);
		print qq!<img src="/pipeline/$res_dir/$dataanalysis_date/plots/id/base_peak/$basepeak_image.mgf.basepeak.txt.labelled.basepeak.png"><br>!;
	}
	else
	{
		print qq!<img src="/pipeline/$res_dir/$dataanalysis_date/plots/id/base_peak/$data_files_.mgf.basepeak.txt.labelled.basepeak.png"><br>!;
	}
	
	system(qq!del "*.Rinfile.txt"!);
	system(qq!del "*.Routfile.txt"!);
	################  zooming ends here ##############
}
1;