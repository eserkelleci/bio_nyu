...
$iterations=1;
open(OUT, ">$DBfilename.$number_of_fragments.$phospho.$charge.$ion_types.$srand.mgf");
					...
					if ($number_of_fragments_=~/([0-9]+)/)
					{
						if (2*(length($peptide)-1)>$number_of_fragments_)
						{
							...
							for($iter=0;$iter<$iterations;$iter++)
							{
								...
								print OUT qq!TITLE=$peptide, MH=$MH, z=$charge, mz=$mz $ion_types, phospho=$phospho_index_ ($phospho_index_all_), $number_of_fragments_ ($fragments)\n!;
								print OUT qq!PEPMASS=$mz\n!;
								print OUT qq!CHARGE=$charge+\n!;
								foreach $fragment (sort numerically @fragments_)
								{
									print OUT "$fragment 100\n";
								}
								print OUT qq!END IONS\n\n!;
								$files_count++;
