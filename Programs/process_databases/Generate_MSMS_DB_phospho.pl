...
sub write_mgf
{
	...
	foreach $peptide (keys %PEP)
	{
		if ($done_this_proteins==0)
		{
				...
				if (abs($MH-$parent_mass)<=$parent_mass_error and $ok==1)
				{
					...
					foreach $number_of_fragments_ (@number_of_fragments)
					{
						...
						if ($number_of_fragments_=~/([0-9]+)/)
						{
							...
							{
								print "Error: $peptide $number_of_fragments_max>$fragments\n";
							}
							else
							{
...
print "$DBfilename, Mp=$parent_mass, Mp-error=$parent_mass_error, z=$charge, $ion_types, $srand $peptides_count\n";