...
if ($error==0)
{
	...
	while ($line=<IN> and $error==0)
	{
		...
		if ($started==1 and $line=~/\w/ and $line!~/END OF FILE/)
		{
			...
			my $locus = $values[$index{"Spectrum name"}];
			#print qq!$locus\n!;
			if ($filter!~/\w/ or $spectrum_name=~/$filter/)
			{
				...
				while ($proteins=~s/^([^\,]+)\,//)
				{
					...
					if ($scores_peptide{"$pep"}<$mascot_score or $scores_peptide{"$pep"}!~/\w/) { $peptide_locus{"$pep"}=$locus; $scores_peptide{"$pep"}=$mascot_score; }
					if ($scores_locus{"$locus"}<$mascot_score or $scores_locus{"$locus"}!~/\w/) { $locus_peptide{"$locus"}=$pep; $scores_locus{"$locus"}=$mascot_score; }
					if ($protein!~/\-R$/)
					{
						if ($scores{"$locus"}<$mascot_score or $scores{"$locus"}!~/\w/) { $scores{"$locus"}=$mascot_score; }
					}
					else
					{
						if ($scores_rev{"$locus"}<$mascot_score or $scores_rev{"$locus"}!~/\w/) { $scores_rev{"$locus"}=$mascot_score; }
					}
				}
			}
		}
	...
	my @to_sort=();
	my $count_pep=0;
	foreach my $locus (keys %scores_locus)
	{
		if ($scores{$locus}=~/\w/)
		{
			if ($scores{"$locus"}>=$mascot_threshold)
			{
				$to_sort[$count_pep++]=qq!$scores{$locus}#$locus_peptide{$locus}##T!;
			}
		}
		else
		{
			if ($scores_rev{$locus}=~/\w/)
			{
				if ($scores_rev{"$locus"}>=$mascot_threshold)
				{
					$to_sort[$count_pep++]=qq!$scores_rev{$locus}#$locus_peptide{$locus}##F!;
				}
			}
		}
	}
