...
if ($ARGV[1]=~/\w/) { $method="$ARGV[1]";} else { $method="1e-3 MS1 MS2"; } # $method="1e-3 MS1"; $method="1e-3";
...
if ($error==0)
{
	...
	if (open(OUT,">$filename_exp_"))
	{
		...
		foreach my $group (@{$experiment->{group}})
		{
			...
			foreach my $sample (@{$group->{sample}})
			{
				...
				foreach my $file (@{$sample->{file}})
				{
					...
					{
						$filename=~s/^.*[\/\\]([^\/\\]+)$/$1/;
						system(qq!D:\\Programs\\identification\\mass_error_xtandem.pl "$local_dir/$filename" "$method"!);
						my $method_=$method;
						$method_=~s/^([0-9\.\-\+edED]+)\s+//; 
						if ($method_=~/\w/)
						{
							system(qq!D:\\Programs\\identification\\recalibrate.pl "$local_dir/$filename" "$method_"!);
						}
						if ($del=~/^del/) { system(qq!del "$local_dir_\\$filename"!); }
						my $filename_=$filename;
						$filename_=~s/\.xml$/_cal.MGF/i;
		...
		print OUT qq!</experiment>\n!;
		close(OUT);
		system(qq!D:\\Programs\\identification\\id_organize_plots.pl "$local_dir"!);
	}
}
