...
if ($error==0)
{
	...
	if (open(OUT,">$filename_exp_"))
	{
		...
		{
			for(my $k=$j+1; $k<@files; $k++)
			{
				system(qq!D:\\Programs\\identification\\time_alignment.pl "$local_dir/$files[$j]" "$local_dir/$files[$k]"!);
			}
		}
		for(my $j=0; $j<@files; $j++)
		{
			if ($del=~/^del/) { system(qq!del "$local_dir_\\$files[$j]"!); }
		}
		print OUT qq!</experiment>\n!;
		close(OUT);
	}
