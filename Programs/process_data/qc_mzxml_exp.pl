...
if ($ARGV[1]=~/\w/) { $method="$ARGV[1]";} else { $method="2 CID"; }
...
if ($error==0)
{
	...
	if (open(OUT,">$filename_exp_"))
	{
			...
			foreach my $sample (@{$group->{sample}})
			{
				...
				foreach my $file (@{$sample->{file}})
				{
					...
					if ($filename=~/\.mzXML$/i)
					{
						$filename=~s/^.*[\/\\]([^\/\\]+)$/$1/;
						#print qq!$filename\n!;
						system(qq!D:\\Programs\\process_data\\qc_mzxml.pl "$local_dir/$filename"!);
		...
		print OUT qq!</experiment>\n!;
		close(OUT);
		system(qq!D:\\Programs\\process_data\\qc_organize_plots.pl "$local_dir"!);
	}
}
